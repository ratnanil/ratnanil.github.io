...
app_ui = ui.page_fluid(
    ui.row(
        ui.input_numeric("f","Price from",100000,min = 1),
        ui.input_numeric("t","Price to",1000000, min = 1),
    ),
    ui.row(
        ui.input_numeric("pk","Pensionskasse",100000, min = 0),
        ui.input_numeric("salary","Salary",130000, min = 0),
    ),
    ui.row(
...
def server(input, output, session):
    ...
    @reactive.Calc
    def price():
        by = (input.t()-input.f())/100
        return([x for x in range(input.f(),input.t(),int(by))])

    @reactive.Calc
    def cash():
        cash = {str(perc): [get_hard_cash(x, input.salary(), input.pk(), perc) for x in price()] for perc in [0.30,0.33,0.36]}
        return(cash)
    ...
    def _():
        scatterplot.data = []
        scatterplot.add_scatter(x=price(), y=cash()["0.33"], mode="markers+lines",name = "33%")
        scatterplot.add_scatter(x=price(), y=cash()["0.3"], mode="markers+lines", name = "30%")
        scatterplot.add_scatter(x=price(), y=cash()["0.36"], mode="markers+lines", name = "36%")
